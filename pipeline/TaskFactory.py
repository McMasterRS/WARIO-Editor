...
class TaskFactory():
    ...
    def __init__(self, type_list):
        ...
        for task_type in type_list:
            try:
                self.loaded_task_types[task_type] = importlib.import_module('tasks.%s' % task_type, "tasks")
                self.loaded_task_types[task_type] = getattr(self.loaded_task_types[task_type], task_type)
            except NotImplementedError:
                print("This is not a module I know")

    def create_task(self, task_type):
        return self.loaded_task_types[task_type]